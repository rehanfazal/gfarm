              ...
              @foreach($services as $sCats)
                <tr>
                  ...
                  <td>
                    ...
                    @if($sCats->status == 1)
                    <a href="{{ route('admin-change-status',['cat_name'=>1,'cat_id'=>$sCats->id,'status'=>0]) }}" msg="Are you sure to deactivate this service?" title="Deactivate">
                      <i class="fa fa-thumbs-o-down" aria-hidden="true"></i>
                    </a>
                          &nbsp;
                    @elseif($sCats->status == 0)
                    <a href="{{ route('admin-change-status',['cat_name'=>1,'cat_id'=>$sCats->id,'status'=>1]) }}" msg="Are you sure to activate this service?" title="Activate">
                    ...
                    <div class="modal" id="myEditModal{{ $sCats->id }}" tabindex="-1" role="dialog">
                          ...
                          <form action="{{ route('admin-main-add-service') }}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                  <label>Name:</label>
                                  <input type="text" class="form-control" placeholder="Enter Service Name" name="service_name" value="{{ $sCats->name }}" required>
                                </div>
                      ...
                      <a href="{{ route('admin-delete-sub-category',['cat_name'=>1,'cat_id'=>$sCats->id]) }}" class="delete_record" msg="Are you sure to delete this service?" title="Delete">
